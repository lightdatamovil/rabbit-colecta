...
export async function handleExternalNoFlex(dbConnection, dataQr, companyId, userId, profile, autoAssign) {
    try {
        ...
        const estado = await executeQuery(dbConnection, querySelectEstadoEnvio, [didenvio]);
        ...
        await updateLastShipmentState(dbConnection, didenvio);
        await informePro(dbConnection, profile, userId);
